...
class EmbeddingFactory:
    ...
    def get_embedding(
    ...
    ) -> Union[GoogleGenerativeAIEmbeddings, OllamaEmbeddings]:
        """Return an embedding model instance for the given provider.
        ...
        if provider == "ollama":
            ...
            return GoogleGenerativeAIEmbeddings(
                model=self.google_model, google_api_key=self.google_api_key
            )
        else:
            raise ValueError(
                f"Provider {provider} không được hỗ trợ. Hãy chọn 'google' hoặc 'ollama'."
            )
